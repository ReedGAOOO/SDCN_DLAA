...
"""
Preprocessing script: Converts an adjacency matrix containing actual distances
into edge features (edge_attr) and a sparse graph structure.
...
- Binary adjacency matrix of the graph structure (scipy sparse matrix) - for compatibility or visualization
- Edge index (numpy array [2, num_edges]) - core graph structure
- Edge feature matrix (numpy array [num_edges, edge_dim]) - generated based on distance
"""
...
def load_distance_matrix(file_path):
    ...
    # print(f"First 5 rows and 5 columns of data:\n{df.iloc[:5, :5]}") # Optional: Less verbose
...
def create_threshold_graph(distance_matrix, theta):
    """
    Creates a threshold-based sparse graph from the distance matrix (keeps edges <= theta).
    ...
    # Find edges with distance in (0, theta]
    # Note: We exclude edges with distance 0 (usually diagonals)
    rows, cols = np.where((distance_matrix > 0) & (distance_matrix <= theta))

    if rows.size == 0:
         print("Warning: No edges found with the current threshold!")
    ...
    # Create binary sparse adjacency matrix (for saving or visualization)
...
def create_knn_graph(distance_matrix, k=10):
    """
    Creates a KNN graph from the distance matrix (keeps only the k nearest neighbors for each node).
    Note: The resulting graph might not be symmetric; only keeps edge i -> j if j is a KNN of i.

    Args:
        distance_matrix: Distance adjacency matrix (NumPy dense matrix).
        k: Number of nearest neighbors to keep for each node.
    ...
    # Copy the distance matrix and set diagonal to infinity to ignore self-loops
    dist_matrix_no_diag = distance_matrix.copy()
    np.fill_diagonal(dist_matrix_no_diag, np.inf)

    for i in range(num_nodes):
        # Find all distances for node i and get sorted indices
        neighbor_distances = dist_matrix_no_diag[i, :]
        nearest_indices = np.argsort(neighbor_distances) # Ascending sort
        ...
        for neighbor_idx in selected_neighbors:
           # Ensure the distance is valid (not inf from the original diagonal)
           dist = neighbor_distances[neighbor_idx] # Check using the modified distances
           if np.isfinite(dist):
                rows.append(i)
...
def normalize_edge_features(edge_attr, method='minmax'):
    """Normalizes edge features (input should be raw distances)."""
    print(f"Normalizing edge features (raw distances) using {method} method")

    if edge_attr.shape[0] == 0:
        print("Edge features are empty, skipping normalization.")
        return edge_attr
    ...
    if method == 'minmax':
        ...
        if max_val == min_val:
             print("Warning: All edge feature values are the same. MinMax normalization results in 0.")
        ...
        std_val = np.std(edge_attr)
         # Avoid division by zero
        if std_val == 0:
            print("Warning: All edge feature values are the same. Standardization results in 0.")
            normalized_edge_attr = np.zeros_like(edge_attr)
        else:
...
def expand_edge_features(normalized_edge_attr, dim=10):
    """Expands edge feature dimension (input should be normalized features)."""
    print(f"Expanding edge feature dimension to {dim}")

    if normalized_edge_attr.shape[0] == 0:
        print("Edge features are empty, skipping expansion.")
        return np.zeros((0, dim))
    if normalized_edge_attr.shape[1] == dim:
        print("Edge feature dimension already meets the requirement, no expansion needed.")
        return normalized_edge_attr
    if normalized_edge_attr.shape[1] != 1 :
        print(f"Warning: Expected 1D features for expansion, but received {normalized_edge_attr.shape[1]}D features. Will use only the first dimension.")


    if dim == 1:
        return normalized_edge_attr[:, 0:1] # Ensure it's [N, 1]

    num_edges = normalized_edge_attr.shape[0]
    expanded_edge_attr = np.zeros((num_edges, dim))

    # Get the first dimension feature (assumed to be normalized distance or its representation)
    base_feature = normalized_edge_attr[:, 0].flatten()

    # Place the base feature in the first column
    expanded_edge_attr[:, 0] = base_feature

    # Generate other features using functions of the base feature
    for i in range(1, dim):
        if i % 3 == 0:
            expanded_edge_attr[:, i] = np.power(base_feature, 2) # Square
        elif i % 3 == 1:
            expanded_edge_attr[:, i] = np.exp(-base_feature * 5) # Exponential decay (multiply by 5 for more distinction)
        else:
            # Avoid division by zero, add a small value
...
def save_processed_data(output_dir, node_features, sparse_adj_binary, edge_index, edge_attr_final, feature_names=None):
    ...
    with open(os.path.join(output_dir, 'data_info.txt'), 'w') as f:
        ...
        f.write(f"Node feature dimension: {node_features.shape[1]}\n")
        f.write(f"Number of edges: {edge_index.shape[1]}\n")
        f.write(f"Final edge feature dimension: {edge_attr_final.shape[1] if edge_attr_final.ndim > 1 else 1}\n")
...
def visualize_data(output_dir, node_features, sparse_adj_binary, edge_attr_final):
    ...
    # 1. Node feature distribution (first 5)
    ...
    # 3. Final edge feature distribution (first 5 dimensions)
    ...
    if sparse_adj_binary.nnz > 0:
        # Calculate out-degrees and in-degrees (might differ for non-symmetric KNN)
...
def main():
    ...
    parser.add_argument('--node_features', type=str, default='NEWDATA/X_simplize.CSV', help='Path to the node feature CSV file')
    parser.add_argument('--distance_matrix', type=str, default='NEWDATA/A.csv', help='Path to the distance adjacency matrix CSV file')
    ...
    parser.add_argument('--k', type=int, default=10, help='Number of neighbors to keep for each node in KNN method')
    parser.add_argument('--theta', type=float, default=None, help='Distance threshold for Threshold method (needs to be set based on data distribution)') # Default None, must be set if method='threshold'

    # Edge feature processing arguments
    parser.add_argument('--normalize', type=str, default='minmax', choices=['minmax', 'standard', 'none'], help='Normalization method for edge features (distances)')
    ...
    print(f"Node feature file: {args.node_features}")
    ...
    print(f"Final edge feature dimension: {args.edge_dim}")
    ...
    if args.method == 'knn':
        ...
        raise ValueError(f"Unknown sparsification method: {args.method}") # Should not happen due to choices
    ...
    print(f"Data preprocessing ({args.method} method) completed")
