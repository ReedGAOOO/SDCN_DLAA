...
from sdcn_spatial_optimized import SDCN_Spatial, target_distribution, eva, train_sdcn_spatial_custom
...
# 优化版本的自定义训练函数
def train_sdcn_spatial_custom_optimized(dataset, adj, args, edge_attr=None):
    """
    训练优化版本的SDCN_Spatial模型
    ...
    results_df.to_csv('spatial_training_results_optimized.csv', index=False)
    
    print("训练完成。结果已保存到 'spatial_training_results_optimized.csv'.")
    
    final_results_df = pd.DataFrame({'节点ID': np.arange(len(final_clusters)), '聚类ID': final_clusters})
    final_results_df.to_csv('spatial_final_cluster_results_optimized.csv', index=False)
    
    print("最终聚类结果已保存到 'spatial_final_cluster_results_optimized.csv'.")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='使用NEWDATA/processed中处理好的数据训练优化版SDCN_Spatial模型',
    ...
    # 训练优化版模型
    print("\n开始训练优化版SDCN_Spatial模型...")
    try:
        model, results, clusters = train_sdcn_spatial_custom_optimized(dataset, adj, args, edge_attr)
