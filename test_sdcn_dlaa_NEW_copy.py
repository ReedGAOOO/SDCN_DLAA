...
def precompute_edge_to_edge_graph(adj, max_edges_per_node=10, device=None):
    ...
    Args:
        ...
        max_edges_per_node: Maximum number of edges considered per node
        device: Device (CPU or GPU)
    ...
    """
    print("Precomputing edge-to-edge graph relationships (one-time operation)...")
    ...
    for node, connected_edges in node_to_edges.items():
        if len(connected_edges) > 1:
            # Randomly sample when exceeding the maximum edge limit
    ...
    print(f"Edge-to-edge graph construction complete: {edge_to_edge_index.shape[1]} edge pairs connected")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Print full exception stack trace to help debugging
        import traceback
        traceback.print_exc()
